# 詠印魔道（Ei-In-Madou）ゲーム概要

**詠印魔道**は、「**詠（ボイス）**」と「**印（モーション）**」を組み合わせて魔法を発動するアクション・キャスティングゲームである。
プレイヤーは手持ちのモーション・コントローラで**印（ジェスチャ）**を描き、同時に無線マイクで**詠唱（ボイス）**を入力する。ゲームは両者の時系列・パターン一致度を評価して魔法の種類・威力・失敗/暴発を判定し、**触覚（振動）**とエフェクトでフィードバックする。

* **印（Motion）**：スイング／ガード／ポーズ等の動作を、世界座標系の加速度・姿勢から判定。
* **詠（Voice）**：2 本の無線マイクを PC 側で **COM ポート**経由の PCM として同時取り込みし、音声処理（NR/AGC/キーワード検出）で詠唱を確定。
* **フィードバック**：命中や詠唱成功時にコントローラへハプティクス（PWM 駆動）を返す。
* **マルチ**：1P/2P の両手・協働詠唱も想定。

---

# Magicling_code

モーション入力＆ハプティクス付き **無線コントローラ**（ESP32 + BNO055）を中心としたコード群である。
